...
class UploadController
{
    ...
    public function __construct(
        ...
        $this->options = array(
            ...
            'redirect_allow_target'            => '/^' . preg_quote(
                    parse_url($this->get_server_var('HTTP_REFERER'),
                        PHP_URL_SCHEME)
                    . '://'
                    . parse_url($this->get_server_var('HTTP_REFERER'),
                        PHP_URL_HOST)
    ...
    protected function get_download_url(
        ...
        if (empty($version)) {
            ...
            if ($version_url) {
                return $version_url . $this->get_user_path()
                    . rawurlencode($file_name);
            }
    ...
    protected function imagick_create_scaled_image(
        ...
        if ($crop) {
            ...
            if (($img_width / $img_height) >= ($max_width / $max_height)) {
                $new_width
                    = 0; // Enables proportional scaling based on max_height
                $x = ($img_width / ($img_height / $max_height) - $max_width)
                    / 2;
            } else {
                $new_height
                    = 0; // Enables proportional scaling based on max_width
        ...
        $success = $image->resizeImage(
            ...
            $new_width
            && $new_height // fit image into constraints if not to be cropped
        );
        if ($success && $crop) {
    ...
    protected function imagemagick_create_scaled_image(
        ...
        $resize = @$options['max_width']
            . (empty($options['max_height']) ? ''
                : 'X' . $options['max_height']);
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    $location = $request->file('cv_location')
                        ->store('careers.tlskills/app/images/forms');
                    $user = Resume::where('resume_id', Auth::user()->user_id)
                        ->update([
    ...
    public function generate_response($content, $print_response = true)
    {
        ...
        if ($print_response) {
            ...
            if ($redirect
                && preg_match($this->options['redirect_allow_target'],
                    $redirect)
            ) {
            ...
            if ($this->get_server_var('HTTP_CONTENT_RANGE')) {
                ...
                if ($files && is_array($files) && is_object($files[0])
                    ...
                    $this->header('Range: 0-' . (
                            $this->fix_integer_overflow((int)$files[0]->size)
                            - 1
                        ));
                }
            }
    ...
    protected function basename($filepath, $suffix = null)
    {
        ...
        return substr(basename('X' . $splited[count($splited) - 1], $suffix),
            1);
    }
}
